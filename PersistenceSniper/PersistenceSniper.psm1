...
function Find-AllPersistence { 
  ...
  .PARAMETER LogType
  Optional, declare the type output format: CSV, JSON or EVENT

  .PARAMETER Destination
  Optional, if the log type is CSV or JSON, identifies the filename. Otherwise it identifies the EventSourceName 
  ...
  .EXAMPLE
  Find-AllPersistence -LogType CSV -Destination .\persistences.csv
  Enumerate low false positive persistence techniques implanted on the local machine and output to a CSV.

  .EXAMPLE
  Find-AllPersistence -LogType JSON -Destination .\persistences.json
  Enumerate low false positive persistence techniques implanted on the local machine and output to a JSON.

  .EXAMPLE
  Find-AllPersistence -LogType EVENT -Destination 'PersistenceSniper'
  Enumerate low false positive persistence techniques implanted on the local machine and output to ETW Application Event Log. The Destination argument identifies the log source name.
  ...
  .EXAMPLE
  Find-AllPersistence -DiffCSV .\persistences.csv -LogType CSV -Destination .\newPersistences.csv
  ...
  .EXAMPLE
  Find-AllPersistence -ComputerName @('dc1.macrohard.lol', 'dc2.macrohard.lol') -IncludeHighFalsePositivesChecks -DiffCSV .\persistences.csv -LogType CSV -Destination .\findings.csv
  ...
  .EXAMPLE
  Find-AllPersistence -ComputerName (Get-Content computers.txt) -IncludeHighFalsePositivesChecks -DiffCSV .\persistences.csv -LogType CSV -Destination .\findings.csv
  ...
  .EXAMPLE
  Find-AllPersistence -DiffCSV .\persistences.csv -LogType CSV -Destination .\findings.csv | Where-Object Classification -Like "MITRE ATT&CK T*"
...
#>
  ...
  if ($Destination -and $LogType -and $persistenceObjectArray.Count -gt 0) {
